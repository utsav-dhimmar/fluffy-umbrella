...
export function CinemaDeck() {
	...
	const localResults = dedupeCatalog(
		curatedCatalog.filter(item => {
			const modeMatches =
				searchMode === "all" || item.kind === searchMode;
			const queryMatches =
				!normalizedQuery ||
	...
	return (
		<div className="mx-auto flex min-h-screen w-full max-w-400 flex-col gap-6 px-4 py-4 text-stone-100 md:px-6 md:py-6">
			<section className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_360px]">
				...
				<aside className="panel flex flex-col">
					<div className="border-b border-white/10 px-4 py-4">
						...
						<h2 className="font-display text-2xl tracking-[0.06em] text-stone-50">
							Search by name
						</h2>
					</div>
					...
					<div className="space-y-4 p-4">
						...
						<div className="flex gap-2">
							{(["tv", "all", "movie"] as SearchMode[]).map(
								mode => (
									<button
										className={
											mode === searchMode
						...
						<div className="rounded-[22px] border border-white/10 bg-black/25 p-3 text-xs leading-5 text-stone-400">
							Lightweight behavior: curated titles render
							instantly, then TMDB search results are fetched
					...
					<div className="flex-1 space-y-3 overflow-y-auto px-4 pb-4">
						{results.length === 0 ? (
							<div className="rounded-[22px] border border-dashed border-white/10 bg-white/5 p-4 text-sm text-stone-400">
								No title matched this query yet. Try a broader
								name or switch the filter.
							</div>
						) : (
							results.map(item => {
								const isActive =
								...
								return (
									...
									>
										<div className="flex items-start justify-between gap-3">
											<div>
												<p className="font-display text-xl leading-none text-stone-50">
													{item.title}
												</p>
											...
											<span className="rounded-full border border-white/10 px-2 py-1 text-[10px] uppercase tracking-[0.22em] text-amber-200">
												{item.badge}
											</span>
										</div>
					...
					<div className="border-t border-white/10 px-4 py-3 text-xs leading-5 text-stone-500">
